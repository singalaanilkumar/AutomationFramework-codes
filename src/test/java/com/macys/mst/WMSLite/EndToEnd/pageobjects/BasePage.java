...
public class BasePage extends PageObject{
	...
	By searchByInputs = By.xpath(".//*[@id='cdk-accordion-child-0']/div/form/div");

	@FindBy(xpath = "\n" +
			"//mat-icon[@class='cursor-arrow unselectable mat-icon notranslate material-icons mat-icon-no-color'][@role='img']\n")
	WebElement matIcon;

	@FindBy(xpath = "//div[@class='popup-box']")
	WebElement menuPopupBox;

	@FindBy(xpath = "//span[text()='clear'][@role='img']")
	WebElement clearPopup;
	...
	@FindBy(xpath = "//div[@class='popup-box']//tr[1]//input")
	private WebElement userName;

	@FindBy(xpath = "//div[@class='popup-box']//tr[2]//input")
	...
	@FindBy(xpath = "//*[contains(text(),'Welcome to Joppa19!')]")
	...
	public void loginWmsLite() {
		driver.get(ReadHostConfiguration.WMS_LITE_URL.value());
		log.info(driver.getCurrentUrl());
		if(driver.getCurrentUrl().contains("login")){
			getWait(20).ignoring(Exception.class).until(ExpectedConditions.visibilityOf(userName));
			userName.clear();
			userName.sendKeys("BH18247" );

			getWait(20).ignoring(Exception.class).until(ExpectedConditions.visibilityOf(password));
			password.clear();
			//log.info(ExecutionConfig.appUIPassword);
			password.sendKeys("Anil@0505");

			getWait(20).ignoring(Exception.class).until(ExpectedConditions.visibilityOf(loginButton));
			loginButton.click();

			getWait(20).ignoring(Exception.class).until(ExpectedConditions.visibilityOf(welcomeText));
			StepDetail.addDetail("User logged in successfully to Joppa19",true);
			}
		else{
			Assert.fail("Logging to Joppa19 failed");
		}


	}

	public void clickWMsLitemenu()throws InterruptedException{
		getWait(1000).ignoring(Exception.class).until(ExpectedConditions.visibilityOf(matIcon));
		Thread.sleep(3000);
		matIcon.click();
		StepDetail.addDetail("Menu is clicked",true);
	}
	public void displayPopupBox()throws InterruptedException{
		getWait(1000).ignoring(Exception.class).until(ExpectedConditions.visibilityOf(menuPopupBox));
		Thread.sleep(3000);
		boolean menuPopup=menuPopupBox.isDisplayed();
		StepDetail.addDetail("Menu item popup is displayed",menuPopup);
	}

	public String getConfigTestData(String dataTag){
		return FileConfig.getInstance().getStringConfigValue("testData." + dataTag);
	}
